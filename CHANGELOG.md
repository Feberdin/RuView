...
### Fixed
- **Unraid Docker network no longer overlaps LAN/VLAN routes.** The Broker
  deployment now pins RuView's default bridge to `10.200.14.0/24` instead of
  allowing Docker to choose a potentially conflicting `192.168.0.0/16`
  subnet. The sensing server now also has a bounded Docker healthcheck against
  its existing `/health/ready` endpoint, so deployments fail clearly when the
  process starts but the API never becomes ready.
