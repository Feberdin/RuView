...
use rumqttc::{AsyncClient, ClientError, EventLoop, MqttOptions, QoS, TlsConfiguration, Transport};
...
fn build_mqtt_options(cfg: &MqttConfig) -> MqttOptions {
    ...
    if !matches!(cfg.tls, TlsConfig::Off) {
        // Use the operating system trust store through the native TLS backend.
        // This keeps certificate validation enabled while avoiding rumqttc's
        // currently unpatched rustls-webpki 0.102 dependency line.
